...
m2 <- robu(formula = final_e_size ~ 1 + ran_colors_objects, data = dat2,
           studynum = umbrella, var.eff.size = vi)
...
for_table <- list(all_intercept, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14)

results_table <- function(l){
  z <- data.frame()
  for(i in 1:length(l)){
    z1 <- data.frame(
      model = format(l[[i]][["ml"]]),
...
output <- results_table(for_table)
