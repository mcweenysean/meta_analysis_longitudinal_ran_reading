...
x <- readxl::read_xlsx("title_screened_import_redcap_4_21_2020.xlsx")
...
x$record_id <- paste0("[S1] ", author_last_name, ", ", x$year, "; ", str_extract(x$title, "^.{20}"), " [2]")
x$record_id <- gsub("'", "", x$record_id)

x$notes <- ""

write.csv(x, "to_upload2_4_21_2020.csv", row.names = F)
