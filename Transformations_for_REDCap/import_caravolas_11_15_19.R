...
data_abstrackr <- read.csv("abstrackr_caravolas_include_11_15_19.csv") %>%
...
data_include <- data_include %>%
  filter(source_id == 'G8GAEVWN')


write.csv(data_include, file = "import_caravolas_11_15_19.csv", row.names=FALSE)
...
write.csv(data_include, file = "import_all419_11_15_19.csv", row.names=FALSE)
